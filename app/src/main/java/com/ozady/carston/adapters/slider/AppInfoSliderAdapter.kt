...
import com.ozady.carston.views.login_system.SplashLogin2Activity
...
class AppInfoSliderAdapter (var context: Context?): SliderViewAdapter<AppInfoSliderAdapter.SliderAdapterVH>() {
    ...
    override fun destroyItem(container: View, position: Int, `object`: Any) {
        if (position == 6){
            context?.startActivity(Intent(context, SplashLogin2Activity::class.java).addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)
